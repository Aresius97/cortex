...
**Attack 1** assumed the FRAUDAR-lite defense was vulnerable to popular-entity dilution in the way a *sum-based* scorer would be. The implementation does not use a sum-based scorer — that approach was removed in Phase 2 Entry 5 specifically because summing rewarded exactly this kind of dilution. The actual scorer uses **mean** edge weight, where popular (low-weight) entities pull an account's average *down*, making it look *more* suspicious, not less. Attack 1 therefore targeted a defense that was no longer in use. The corrected version (1b) — pairing ring members with fresh decoy accounts on new degree-2 devices, mimicking the honest population's own high-scoring pattern — achieves genuine, full evasion.
...
**The lesson, stated directly:** an attack failing does not mean a defense is robust. It might mean the attack targeted the wrong mechanism. Each failed attack was diagnosed for its actual failure mode, using the same rigor applied to detector bugs, rather than being treated as evidence of robustness.

## A deeper finding: detection success doesn't guarantee the case survives downstream — and the fix

Attack 1, despite being **caught at 100% recall** by FRAUDAR-lite, showed **zero exposure** in the Phase 6 priority queue when first evaluated. Diagnosis: Attack 1's ring shares a device used by 7 of its 10 members — degree 7, which exceeds `CASE_ENTITY_DEGREE_CAP=4`, the cap Phase 6 introduced specifically to stop case-clustering from collapsing into a mega-cluster (Entry 11). That cap was tuned against the baseline dataset's typical ring density and had not been tested against a ring this dense.

**This was fixed, not just reported — completing the full loop: attack finds gap → fix system → re-attack → measure improvement.** The fix required three layered corrections (`ENGINEERING_LOG.md` Entry 14), rather than a single change:
...
3. Fixing #2 immediately caused precision on selected cases to collapse to 1/60, because matching on shared device/IP alone counts an account's *entire* transaction history through that device, including unrelated legitimate spending. The exposure attribution was therefore corrected to require a transaction's counterparty to also be shared by another case member — the actual signature of coordinated abuse.

**Measured result of re-running the full pipeline after the fix:** captured exposure at the same 5-investigator budget rose from ₹953,469 (counterfactual, no adversary) to **₹1,021,532 (post-attack) — a +7.1% increase**, achieved with *fewer* selected cases (13 vs. 15). This is not attributable to simple volume fluctuation: Attack 1's now-correctly-valued case earned a spot in the selected set and displaced a lower-value case entirely, while the top-5 ranking stayed stable (still the highest-value genuine FlowScope rings). Catching an attack and *correctly valuing* what was caught measurably improved the system's overall efficiency, not just its recall.
